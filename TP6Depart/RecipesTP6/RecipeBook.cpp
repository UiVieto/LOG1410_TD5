...
Recipe* RecipeBook::findRecipe(std::string keyword)
{
	...
	for (auto it = m_recipes.begin(); it != m_recipes.end(); ++it)
	{
		if (it->getDescription() == keyword)
		{
			foundRecipe = dynamic_cast<Recipe*>(&(*it));
			break;
		}
	}
...
void RecipeBook::initialize()
{
	...
	Ingredient oignonsHaches("haches", 120, findProduct("oignons"));
	sauceTomate.addIngredient(oignonsHaches);
	...
	Ingredient oignonsTranches("tranches", 120, findProduct("oignons"));
	pizza.addIngredient(oignonsTranches);
